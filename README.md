...
1. **Преобразование входа в пространство признаков**  
   Stem переводит входной тензор размера \(H \times W \times C\) в тензор признаков с большим числом каналов \(C'\), которые используются последующими свёрточными блоками. Это задаёт стартовую размерность признаков всей модели.
...
- $I$ — входной тензор (например, RGB-изображение);
- $\mathrm{Conv}(\cdot)$ — свёрточная операция (обычно с ядром $3\times3$ или $7\times7$);
- $\mathrm{Norm}(\cdot)$ — операция нормализации (BatchNorm или GroupNorm);
- $\phi(\cdot)$ — нелинейная функция активации (ReLU, SiLU и др.);
- $X_0$ — выходной тензор stem, первый уровень признаков сети.


На практике stem может состоять из нескольких последовательно применяемых свёрток $3 \times 3$, что позволяет:
- увеличить выразительность за счёт дополнительных нелинейностей;
- избежать чрезмерно агрессивной свёртки большого размера;
- аккуратнее сохранить локальные детали.

---

### Почему stem важен в CNN-архитектуре

Stem определяет **начальный баланс между детализацией и инвариантностью**:

- слишком агрессивный stem (большой stride или крупное ядро) может разрушить мелкие структуры;
- слишком слабый stem увеличивает вычислительную стоимость и замедляет обучение;
- выбор числа выходных каналов влияет на выразительность всей сети.

Таким образом, stem — это не вспомогательный, а **критически важный архитектурный компонент**, который задаёт форму и качество всех последующих признаков.

---

### Роль stem в текущем проекте

В данном CNN-проекте stem используется как входной модуль, который:
- преобразует входные данные в начальное пространство признаков;
- извлекает низкоуровневые визуальные структуры;
- подготавливает стабильное и информативное представление для дальнейших блоков (attention, gating, residual layers и т.д.).

Именно поэтому backbone модели логически начинается со stem, а не с первого высокоуровневого блока: качество начального представления напрямую влияет на эффективность всей архитектуры.
...
F_{\text{fused}}
=
\alpha \cdot F_{\text{att}}
+
(1 - \alpha) \cdot F_{\text{rgb}}
$$
